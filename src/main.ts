...
window.onload = () => {
  ...
  map = L.map("map").setView([45.2, -63.1], 8);
...
function displaySightings() {
  ...
  request.onsuccess = (event: Event) => {
    ...
    sightings.forEach(sighting => {
      ...
      addMarker(sighting.location.latitude, sighting.location.longitude, sighting.species, sighting.date, sighting.sex, sighting.notes);
    });
  };
}
